...
func TestRandomlyMove(t *testing.T) {
	...
	for _, test := range tests {
		...
		w := World{
			Cities:      test.cities,
			Links:       test.links,
			Aliens:      AliensMap{0: a},
			StuckAliens: AliensMap{},
		}

		city, err := w.RandomlyMove(a.Id)
		if city != test.expectedCity.Name {
			t.Errorf("Expected %v, got %v", test.expectedCity, city)
		}
		...
		if len(w.StuckAliens) != test.expectedStuckNum {
			t.Errorf("Expected %v stuck aliens, got %v", test.expectedStuckNum, len(w.StuckAliens))
		}
	}
}
